...
def parse_factor(tokens):
    ...
    if token["tag"] == "(":
        ast, tokens = parse_arithmetic_expression(tokens[1:])
        assert tokens[0]["tag"] == ")"
        return ast, tokens[1:]
    if token["tag"] == "-":
...
def test_parse_factor():
    ...
    assert tokens == [{'tag': None, 'value': None, 'position': 2}]
    tokens = tokenize("-3+2")
    ast, tokens = parse_factor(tokens)
    assert ast == {'tag': 'negate', 'value': {'tag': 'number', 'value': 3}}
    assert tokens == [{'tag': '+', 'position': 2, 'value': '+'}, {'tag': 'number', 'position': 3, 'value': 2}, {'tag': None, 'value': None, 'position': 4}]
...
def parse_relational_expression(tokens):
    """
    relational_expression = term { "<"|">","<=",">=", "==", "!=" arithmetic_expression }
    """
    node, tokens = parse_term(tokens)
    while tokens[0]["tag"] in ["<", ">", "<=", ">=", "==", "!="]:
        tag = tokens[0]["tag"]
...
def test_parse_relational_expression():
    ...
    print("testing parse_arithmetic_expression()")
    for operator in ["<", ">", "<=", ">=", "==", "!="]:
        tokens = tokenize(f"1{operator}2")
        ast, tokens = parse_relational_expression(tokens)
        assert ast == {'tag': operator, 'left': {'tag': 'number', 'value': 1}, 'right': {'tag': 'number', 'value': 2}}
        assert tokens == [{'tag': None, 'value': None, 'position': 2+len(operator)}]
